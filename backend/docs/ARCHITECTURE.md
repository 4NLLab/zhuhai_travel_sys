...
| `DB_PORT` | `3306` | 数据库端口 |
| `DB_USER` | `root` | 数据库用户 |
| `DB_PASSWORD` | 空 | 数据库密码，必须在服务器环境变量中配置 |
| `DB_NAME` | `zhuhai_travel` | 数据库名 |
| `JWT_SECRET` | `change-me-in-production` | JWT 签名密钥，生产环境必须替换 |
| `TOKEN_TTL_HOURS` | `168` | 登录 token 有效期 |
| `CORS_ALLOWED_ORIGIN` | `*` | 前端域名白名单，生产建议指定域名 |
| `PAYMENT_WEBHOOK_SECRET` | `change-me-payment-webhook-secret` | 支付回调 HMAC 签名密钥 |
...
| POST | `/api/v1/auth/phone-login` | 用户手机号登录/注册，返回 Bearer token |
| POST | `/api/v1/driver/register` | 司机注册，提交身份和车辆信息 |
| POST | `/api/v1/driver/login` | 司机登录，返回 Bearer token |
| POST | `/api/v1/admin/login` | 管理员登录，返回 Bearer token |

### 用户端

| 方法 | 路径 | 说明 | Handler |
|---|---|---|---|
| GET | `/api/v1/users/profile` | 用户信息，用户身份来自 Bearer token | `UserProfile` |
...
| 方法 | 路径 | 说明 | Handler |
|---|---|---|---|
| GET | `/api/v1/travelers` | 列表，用户身份来自 Bearer token | `TravelerList` |
...
| 方法 | 路径 | 说明 | Handler |
|---|---|---|---|
| GET | `/api/v1/favorites` | 收藏列表，用户身份来自 Bearer token | `FavoriteList` |
...
| 方法 | 路径 | 说明 | Handler |
|---|---|---|---|
| GET | `/api/v1/invoice-titles` | 发票抬头列表，用户身份来自 Bearer token | `InvoiceTitleList` |
...
| GET | `/api/v1/orders?status=&page=&size=` | 用户订单列表，用户身份来自 Bearer token | `OrderList` |
| GET | `/api/v1/orders/:id` | 订单详情（子项/票/出游人/支付） | `OrderDetail` |
| POST | `/api/v1/payments/callback` | 支付回调，需 `X-Payment-Timestamp` 和 `X-Payment-Signature` | `PaymentCallback` |
...
| POST | `/api/v1/admin/drivers/review` | 后台审核司机，审核通过后生成车座二维码 | `AdminDriverReview` |
